...
class digraphPlot(tk.Canvas, tk.Frame):
    ...
    def start_girvan_newman(self, graph, depth):
        # Ensure graph is undirected for the algorithm
        ...
        for i in depth:
            ...
            for j in range(i):
                ...
                if communities:
                    # Store sorted nodes for this iteration in DataFrame
                    for k, community in enumerate(communities, start=1):
                        sorted_community = sorted(community)
                        community_df = community_df.append({
                            'Iteration': j+1,
                            'Community Number': k,
                            'Nodes': ', '.join(map(str, sorted_community))
                            }, ignore_index=True)
                    
                    # Assign colors to nodes based on their last known community
                    community_colors = {node: f"C{a}" for a, community in enumerate(communities) for node in community}
